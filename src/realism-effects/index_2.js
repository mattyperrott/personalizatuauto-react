...
import { WebGLMultipleRenderTargets, ShaderMaterial, GLSL3, NoBlending, Uniform, ShaderChunk, ShaderLib, UniformsUtils, Vector2, Matrix4, Vector3, Clock, Quaternion, LinearFilter, HalfFloatType, SRGBColorSpace, FramebufferTexture, RGBAFormat, TextureLoader, NearestFilter, RepeatWrapping, NoColorSpace, WebGLRenderTarget, FloatType, DataTexture, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, RedFormat, Source, Color, Matrix3, TangentSpaceNormalMap, DepthTexture, Texture, NoToneMapping } from 'three'
...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1, temporalReprojectCustomComposeShader = '') {
    let finalFragmentShader = fragmentShader$3.replace('#include <reproject>', reproject).replace('#include <gbuffer_packing>', gbuffer_packing)
...
var blue_noise = '#define GLSLIFY 1\nuniform sampler2D blueNoiseTexture;uniform vec2 blueNoiseSize;uniform int blueNoiseIndex;const vec4 hn=vec4(0.618033988749895,0.3247179572447458,0.2207440846057596,0.1673039782614187);uvec4 s1;ivec2 pixel;void rng_initialize(vec2 p,int index){pixel=ivec2(p);s1=uvec4(index,index*15843,index*31+4566,index*2345+58585);}void pcg4d(inout uvec4 v){v=v*1664525u+1013904223u;v.x+=v.y*v.w;v.y+=v.z*v.x;v.z+=v.x*v.y;v.w+=v.y*v.z;v=v ^(v>>16u);v.x+=v.y*v.w;v.y+=v.z*v.x;v.z+=v.x*v.y;v.w+=v.y*v.z;}ivec2 shift2(ivec2 size){pcg4d(s1);return(pixel+ivec2(s1.xy % 0x0fffffffu))% size;}vec4 blueNoise(int index){rng_initialize(vUv*resolution,index);vec4 blueNoise=texelFetch(blueNoiseTexture,shift2(ivec2(blueNoiseSize)),0);return blueNoise;}vec4 blueNoise(){return blueNoise(int(blueNoiseIndex));}' // eslint-disable-line
...
var fragmentShader$2 =
  '#define GLSLIFY 1\nvarying vec2 vUv;uniform sampler2D inputTexture;uniform sampler2D inputTexture2;uniform sampler2D depthTexture;uniform mat4 projectionMatrixInverse;uniform mat4 cameraMatrixWorld;uniform float radius;uniform float phi;uniform float lumaPhi;uniform float depthPhi;uniform float normalPhi;uniform float roughnessPhi;uniform vec2 resolution;layout(location=0)out vec4 gOutput0;layout(location=1)out vec4 gOutput1;const float e=2.7182818;\n#include <common>\n#include <gbuffer_packing>\n#define luminance(a) log(dot(vec3(0.2125, 0.7154, 0.0721), a));\nvec3 getWorldPos(float depth,vec2 coord){float z=depth*2.0-1.0;vec4 clipSpacePosition=vec4(coord*2.0-1.0,z,1.0);vec4 viewSpacePosition=projectionMatrixInverse*clipSpacePosition;vec4 worldSpacePosition=cameraMatrixWorld*viewSpacePosition;worldSpacePosition.xyz/=worldSpacePosition.w;return worldSpacePosition.xyz;}float distToPlane(const vec3 worldPos,const vec3 neighborWorldPos,const vec3 worldNormal){vec3 toCurrent=worldPos-neighborWorldPos;float d=abs(dot(toCurrent,worldNormal));return d;}void toDenoiseSpace(inout vec3 color){color=log(color+1.);}void toLinearSpace(inout vec3 color){color=exp(color)-1.;}void evaluateNeighbor(const vec4 neighborTexel,const float neighborLuminance,inout vec3 denoised,inout float totalWeight,const float similarity){float w=similarity;if(w<0.01)return;denoised+=w*neighborTexel.rgb;totalWeight+=w;}const vec2 poissonDisk[samples]=POISSON_DISK_SAMPLES;void main(){float depth=textureLod(depthTexture,vUv,0.).r;if(depth==1.0){discard;return;}vec4 texel=textureLod(inputTexture,vUv,0.0);vec4 texel2=textureLod(inputTexture2,vUv,0.0);toDenoiseSpace(texel.rgb);toDenoiseSpace(texel2.rgb);float lum=luminance(texel.rgb);float lum2=luminance(texel2.rgb);float a=texel.a;float a2=texel2.a;float pixelAge=a+a2;Material mat=getMaterial(gBufferTexture,vUv);float roughnessSq=mat.roughness*mat.roughness;vec3 worldPos=getWorldPos(depth,vUv);vec4 random=blueNoise();float angle=random.r*2.*PI;float s=sin(angle),c=cos(angle);mat2 rotationMatrix=mat2(c,-s,s,c);float specularWeight=mat.roughness*mat.roughness;specularWeight*=specularWeight;float w=1./log(a*a+e);float w2=1./log(a2*a2+e);float historyW=smoothstep(0.,1.,w);float historyW2=smoothstep(0.,1.,w2);float totalWeight=1.;float totalWeight2=1.;vec3 denoised=texel.rgb*totalWeight;vec3 denoised2=texel2.rgb*totalWeight2;float r=1.+random.a*exp(-pixelAge*0.001)*radius;for(int i=0;i<samples;i++){vec2 offset=r*rotationMatrix*poissonDisk[i];vec2 neighborUv=vUv+offset;float distanceToCenter=length(offset)+1.;distanceToCenter=pow(distanceToCenter,8.)-1.;Material neighborMat=getMaterial(gBufferTexture,neighborUv);vec4 neighborTexel=textureLod(inputTexture,neighborUv,0.);vec4 neighborTexel2=textureLod(inputTexture2,neighborUv,0.);toDenoiseSpace(neighborTexel.rgb);toDenoiseSpace(neighborTexel2.rgb);float neighborDepth=textureLod(depthTexture,neighborUv,0.0).x;if(neighborDepth==1.0)continue;vec3 neighborWorldPos=getWorldPos(neighborDepth,neighborUv);float normalDiff=1.-max(dot(mat.normal,neighborMat.normal),0.);float depthDiff=10.*distToPlane(worldPos,neighborWorldPos,mat.normal);float roughnessDiff=abs(mat.roughness-neighborMat.roughness);float neighborLuminance=luminance(neighborTexel.rgb);float neighborLuminance2=luminance(neighborTexel2.rgb);float lumaDiff=mix(abs(lum-neighborLuminance),0.,historyW);float lumaDiff2=mix(abs(lum2-neighborLuminance2),0.,historyW2);float basicWeight=exp(-normalDiff*normalPhi-depthDiff*depthPhi-roughnessDiff*roughnessPhi);float similarity=historyW*pow(basicWeight,phi/historyW)*exp(-lumaDiff*lumaPhi);float similarity2=historyW2*pow(basicWeight,phi/historyW2)*exp(-lumaDiff2*lumaPhi);similarity2*=mix(exp(-distanceToCenter*20.-normalDiff*100.),1.,roughnessSq*roughnessSq);evaluateNeighbor(neighborTexel,neighborLuminance,denoised,totalWeight,similarity);evaluateNeighbor(neighborTexel2,neighborLuminance2,denoised2,totalWeight2,similarity2);}denoised=totalWeight>0. ? denoised/totalWeight : texel.rgb;denoised2=totalWeight2>0. ? denoised2/totalWeight2 : texel2.rgb;toLinearSpace(denoised);toLinearSpace(denoised2);gOutput0=vec4(denoised,texel.a);gOutput1=vec4(denoised2,texel2.a);}' // eslint-disable-line

function generateDenoiseSamples(numSamples, numRings, texelSize) {
...
class SVGF {
  constructor(scene, camera, velocityDepthNormalPass, textureCount = 1, options = {}) {
    this.svgfTemporalReprojectPass = new TemporalReprojectPass(scene, camera, velocityDepthNormalPass, textureCount, { ...options, fullAccumulate: true, logTransform: true, copyTextures: false })
...
class GBufferPass extends Pass {
  ...
  setMRTMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      var _c$material$metalness, _c$material$roughness, _c$material$emissiveI

      const originalMaterial = c.material
      let [cachedOriginalMaterial, mrtMaterial] = this.cachedMaterials.get(c) || [] // init a new material if the original material changed or if the cached material is missing
      ...
      c.visible = isChildMaterialRenderable(c, originalMaterial) // todo: implement selection
      ...
      mrtMaterial.uniforms.roughness.value = (_c$material$roughness = c.material.roughness) !== null && _c$material$roughness !== void 0 ? _c$material$roughness : 0
  ...
  render(renderer) {
    ...
    this.unsetMRTMaterialInScene() // reset state
...
class SSGIPass extends Pass {
  ...
  render(renderer) {
    this.frame = (this.frame + this.ssgiEffect.spp) % 4096 // render G-Buffers

    this.gBufferPass.render(renderer) // update uniforms
...
class SSGIEffect extends Effect {
  constructor(composer, scene, camera, velocityDepthNormalPass, options) {
    ...
    this.svgf.denoisePass.setDepthTexture(this.ssgiPass.gBufferPass.depthTexture) // patch the denoise pass
...
class VelocityDepthNormalPass extends Pass {
  constructor(scene, camera, renderDepth = true) {
    super('velocityDepthNormalPass') // if (!(camera instanceof PerspectiveCamera)) {
    // 	throw new Error(
    // 		this.constructor.name +
    // 			" doesn't support cameras of type '" +
    // 			camera.constructor.name +
...
export { HBAOEffect, MotionBlurEffect, PoissionDenoisePass, SSDGIEffect, SSGIEffect, SSREffect, SVGF, TRAAEffect, TemporalReprojectPass, VelocityDepthNormalPass, VelocityPass }
